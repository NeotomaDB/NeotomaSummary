...
datasettypes <- function(x) {
  call <- httr::GET('https://api.neotomadb.org/v2.0/data/summary/dstypemonth',
...
mc2 <- fct_collapse(monthly_change$datasettype,
                               biomarker = 'biomarker',
                               charcoal = c('charcoal', 'macrocharcoal', 'microcharcoal', 'charcoal surface sample'),
                               ...
                               sedimentology=c('physical sedimentology','paleomagnetic','X-ray fluorescence (XRF)'),
                               'phytolith' = 'phytolith',
                               'plant macrofossil'='plant macrofossil',
                               pollen = c('pollen', 'pollen trap', 'pollen surface sample'),
                               'stable isotope' = 'specimen stable isotope',
                               'testate amoebae' = c('testate amoebae', 'testate amoebae surface sample'),
...
monthly_out <- monthly_change %>% 
  ...
  mutate(month = (Sys.Date() %m-% months(month)))
...
datasetdbs <- function(x) {
  call <- httr::GET('https://api.neotomadb.org/v2.0/data/summary/dsdbmonth',
                    query=list(start=x, end=x+1))
  result <- jsonlite::fromJSON(httr::content(call, as='text'))$data$data
  result$month <- x
  result$counts <- as.numeric(result$counts)
...
out <- ggplot(monthly_change, aes(x=month, y = counts, fill = databasename)) +
  geom_bar(stat='identity') +
  theme_tufte() +
...
rawmonthly <- function(x) {
  call <- httr::GET('https://api.neotomadb.org/v2.0/data/summary/rawbymonth',
