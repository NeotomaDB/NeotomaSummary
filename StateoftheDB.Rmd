...
```{r pullLogs, echo=FALSE, fig.caption="- API calls per day, split by Explorer vs core. From the structured request logs, which start 2026-07-16, so this grows over time."}

# Rich per-request data (calls, bytes, explorer split) from the App Runner JSON access logs.
# Only exists from 2026-07-16 on; the long-run totals come from metrics (pullLog, below).
pullLogRich <- jsonlite::fromJSON('log_run.json')['results']$results %>%
...
# Daily totals per service from App Runner metrics -- the long history back to the Nov 2025
# migration, including the pre-2026-07-16 stretch when access logs still went to a file.
...
Since structured request logging began on 2026-07-16, the API has served `r p(floor(sum(pullLogRich$volume, na.rm=TRUE) / 1000000000))` GB of data. The per-endpoint and data-volume figures below come from those request logs and so cover 2026-07-16 onward, growing over time; the total call count above reaches back to the November 2025 App Runner migration via CloudWatch metrics, because before 2026-07-16 the API's request logs were written to a file and never reached CloudWatch.
...
# top endpoints so far; head() rather than [1:20,] so it holds up while data is still thin.
topCalls <- head(pullCalls, 20)
ggplot(topCalls) +
  geom_bar(aes(x = seq_len(nrow(topCalls)),
