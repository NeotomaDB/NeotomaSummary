...
```{r plotSites, eval=FALSE, echo=FALSE, warning=FALSE, fig.cap='**Figure 1.** *Locations of newly added sites in Neotoma during the past year. The map is interactive and supports zoom/pan operations. Individual sites can be selected and you will be directed to a link for the [Neotoma Explorer](https://apps.neotomadb.org)*.'}
...
pullLog <- function(filename) {
  aa <- read.delim(filename, 
    ...
    mutate(date = lubridate::round_date(as_datetime(date), unit = 'day'))
  return(aa)
}
...
tiliaLog <- pullLog('~/Documents/Neotoma/logs/ceiwww/tilia.neotomadb.org/node/access.log')
...
```{r modifyTiliaLog, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE}
stewardLog <- tiliaLog %>% 
